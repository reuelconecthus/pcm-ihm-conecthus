...
export const entryPaths = {
    '/api/serial-numbers': { post: {
        ...
        description: 'Valida serial ou QR e grava no MongoDB. Maiúsculas e minúsculas são distintas. Serial e QR com o mesmo valor são o mesmo produto. Não utiliza mensageria. Se a resposta se perder, a gravação pode ter ocorrido; repetir pode retornar 409. Try it out executa uma gravação real quando o MongoDB está configurado.',
        ...
        responses: {
            '200': { description: 'Gravação confirmada no MongoDB.', content: { 'application/json': { schema: { $ref: '#/components/schemas/ApiSuccess' }, example: { status: 'OK' } } } },
            ...
            '503': errorResponse('Banco ausente, indisponível ou gravação não confirmada.', 'Não foi possível confirmar o registro no MongoDB.')
        }
    } }
};
